...
# Uso: cd /opt/brieflow && ./PLAN/full-build.sh
...
if [ -f "server/index.ts" ]; then
    ...
    if [ -f "dist/index.cjs" ]; then
        ...
        echo "   Arquivo: dist/index.cjs ($(du -h dist/index.cjs | cut -f1))"
    else
...
if [ -f "script/build.ts" ]; then
    if npx tsx script/build.ts 2>&1 | tee /tmp/script-build.log; then
...
echo "   - script/build.ts: /tmp/script-build.log"
echo ""
